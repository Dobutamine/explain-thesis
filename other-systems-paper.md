...
closed-loop calibration pipeline described in full in the companion parameterization paper [P6]: a large language model interprets the available clinical targets into a validated, allowlisted specification, and a deterministic calibrator drives one physiologically interpretable lever per target to a clinician-meaningful tolerance; the language model performs no numerical fitting and never edits equations or state. The regulatory models of this paper contribute both **levers** and **guarded loops** to that pipeline. As levers, their set-points and gains individualise a patient (e.g. the renal filtration coefficient *K*_f for GFR; the thermoregulatory environment *T*_env for incubator setting). As guarded loops, they must be *respected* by the calibrator: because the RAAS/ADH axis defends arterial pressure and volume, a naïve pressure lever is opposed by the controller — the same class of interaction the baroreflex creates in the cardiovascular paper [P1] — so the calibrator drives pressure through a resistance lever that composes with, rather than fights, the hormonal effector. The full method, per-target lever assignments and tolerances are given in the parameterization paper [P6].
