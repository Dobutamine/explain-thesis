...
| Source | Role | Mark |
|---|---|---|
| `ExplainCircPaper(27012026)_WPdB_TA_WvM.docx` (Introduction) | 1.1–1.2 background/purpose seed | adapt |
| `docs/engine/README.md`, `docs/engine/ARCHITECTURE.md` | 1.2–1.3 purpose + integration story | adapt |
| `_shared-methods.md` (S1 notation, S2 factor composition, S3 advective transport, S4 cycle counters) | 1.3 integration substrate | adapt |
| `ai-parameterization-paper.md` (Introduction) | 1.4 parameterization problem | adapt |
...
| `_shared-methods.md` (S5 software, S7 reproducibility) | §2.3 software text | reuse |

### Chapter 3 — Respiratory  (Paper 2)
| Source | Role | Mark |
|---|---|---|
| `respiratory-paper.{md,docx}` | entire chapter | reuse |
...
| `Fig1_respiratory_subsystem.{png,svg}` | chapter figure | reuse |

### Chapter 4 — Regulatory organ systems  (Paper 3)
| Source | Role | Mark |
|---|---|---|
| `other-systems-paper.{md,docx}` | entire chapter | reuse |
...
| `Fig1_regulatory_systems.{png,svg}` | chapter figure | reuse |

### Chapter 5 — Devices  (Paper 5)
| Source | Role | Mark |
|---|---|---|
| `devices-paper.{md,docx}` | entire chapter | reuse |
...
| Source | Role | Mark |
|---|---|---|
| `ai-parameterization-paper.{md,docx}` (Tables 1–3, worked convergence trace) | entire chapter | reuse |
| `circ-paper-figure-AIparam.md` + `Fig6_AI_parameterization.{png,svg}` | chapter figure + caption | reuse |
...
| `circ-paper-additions.md` (blocks A, G) | source of the condensed §2.4 that stays in Ch 2 as a pointer | adapt |
...
| Source | Role | Mark |
|---|---|---|
| `_shared-methods.md` (S1–S7) | one consistent description of notation/composition/software/reproducibility, reused verbatim across chapters | reuse |
| `_references.md` | consolidated bibliography (needs verification — §5) | adapt |
| `CHANGELOG_working_docx.md` | record of circ master vs working-copy edits; drives the consistency cleanup | reference |
| `ExplainCircPaper_WORKING_with_AIparam.docx` | shows the merged-in AI-param version of Paper 1 | reference (do not use as canonical) |
...
| Respiratory subsystem schematic | `Fig1_respiratory_subsystem.{png,svg}` | Ch 3 |
| Regulatory-controller layer | `Fig1_regulatory_systems.{png,svg}` | Ch 4 |
| AI-parameterization pipeline (closed loop) | `Fig6_AI_parameterization.{png,svg}` | Ch 6 (+ pointer in Ch 2) |

### New (Fig 1.2 & Fig 7.1 produced 2026-07-10 — house-style SVG masters + 2800×2160 PNGs in the repo root)
...
5. **Legacy inconsistencies in the circ master** — residual "Python" implementation language (the engine is JavaScript/TypeScript in a Web Worker + Vue) and a URL mismatch. → Apply the `circ-paper-additions.md` block E rewrite + CHANGELOG edits; canonical URL is now `explain-modeling.com` (all drafts updated).
