...
- **Equations** are **LaTeX `$$…$$` math blocks** with `\tag{N}` for the number (e.g. `$$a_a(t) = \sin[\pi\,(t/T_{\text{as}})] \tag{1}$$`), so they render as real math in the Obsidian vault and on GitHub; prose refers to them as "Eq. N", and multi-line/piecewise equations fold into `\begin{cases}`/`\begin{aligned}`. Each equation is checked against the named engine source file. Equations are re-keyed as **native Word OMML at assembly** (LaTeX→OMML is straightforward) — do not treat Markdown as the final form. *Converted series-wide from the earlier plain-text-Unicode-blockquote style across P1/P2/P4/P6 on 2026-07-18 (the other papers carry no equations); do **not** revert to plain-text.*
